...
package com.alisacasino.bingo.components
{
	...
	public class Scale9Image extends Sprite implements IValidating
	{
		...
		private var _ensureSeamlessTiling:Boolean = false;
		
		/**
		 * Defines the method of tiling. If set to true, the texture in the tiled area will be stretched to
		 * ensure that it fills the area with a whole number of tiles. That way the points where the tiled area
		 * meets the corners would be consistent with the source texture.
		 *
		 * When set to false, texture will only be scaled according to the textureScale property.
		 *
		 * @default false
		 */
		public function get ensureSeamlessTiling():Boolean
		{
			return _ensureSeamlessTiling;
		}
		
		public function set ensureSeamlessTiling(value:Boolean):void
		{
			_ensureSeamlessTiling = value;
			this._renderingChanged = true;
			this.invalidate();
		}
		 ...
		 * @private
		 */
		/*override public function flatten(ingoreChildOrder:Boolean = false):void
		{
			this.validate();
			super.flatten(ingoreChildOrder);
		}*/
		...
		public function validate():void
		{
			...
			if (this._propertiesChanged || this._layoutChanged || this._renderingChanged)
			{
				...
				helperImage.textureSmoothing = this._smoothing;
		...
		protected function addPart(texture:Texture, x:Number, y:Number, width:Number, height:Number):void
		{
			...
			if (_isTiled)
			{
				...
				if (ensureSeamlessTiling)
				{
					numberOfColumns = Math.max(1, Math.round(numberOfColumns));
					numberOfRows = Math.max(1, Math.round(numberOfRows));
				}
			}
			...
			var needToCutImageByWidth:Boolean;
			var needToCutImageByHeight:Boolean;
			var modifiedWidth:Number = 1;
			var modifiedHeight:Number = 1;
			...
			for (var i:int = 0; i < numberOfRows; i++)
			{
				...
				if (isTiled && i + 1 > numberOfRows)
				{
					needToCutImageByHeight = true;
					modifiedHeight = numberOfRows - i;
				}
				else
				{
					needToCutImageByHeight = false;
					modifiedHeight = 1;
				}
				
				for (var j:int = 0; j < numberOfColumns; j++)
				{
					if (isTiled && j + 1 > numberOfColumns)
					{
						needToCutImageByWidth = true;
						modifiedWidth = numberOfColumns - j;
					}
					else
					{
						needToCutImageByWidth = false;
						modifiedWidth = 1;
					}
					
					helperImage.x = currentX;
					
					helperImage.width = tileWidth*modifiedWidth;
					helperImage.height = tileHeight*modifiedHeight;
					
					if (needToCutImageByWidth || needToCutImageByHeight)
					{
						helperImage.setTexCoords(1, helperImage.width / originalTileWidth, 0);
						helperImage.setTexCoords(2, 0, helperImage.height / originalTileHeight);
						helperImage.setTexCoords(3, helperImage.width / originalTileWidth, helperImage.height / originalTileHeight);
					}
					
					this._batch.addMesh(helperImage);
					
					helperImage.setTexCoords(1, 1, 0);
					helperImage.setTexCoords(2, 0, 1);
					helperImage.setTexCoords(3, 1, 1);
					
					currentX += helperImage.width;
				}
				
				currentX = x;
				currentY += helperImage.height;
			}
		}
